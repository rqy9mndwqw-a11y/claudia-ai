...
import { SUPPORTED_EXCHANGES, type ExchangeId } from "@/lib/exchange-config";
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder={`Your ${SUPPORTED_EXCHANGES.find((e) => e.id === exchange)?.name ?? exchange} API key`}
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder={`Your ${SUPPORTED_EXCHANGES.find((e) => e.id === exchange)?.name ?? exchange} API secret`}
                    className="w-full bg-surface border border-white/10 rounded-lg px-4 py-3
                               text-white text-sm font-mono outline-none focus:border-accent/30"
                  />
                </div>
                ...
                >
                  {verifying ? "Verifying..." : `Connect ${SUPPORTED_EXCHANGES.find((e) => e.id === exchange)?.name ?? exchange}`}
                </button>

                <div className="bg-surface/50 rounded-lg p-3 border border-white/5">
                  <p className="text-zinc-600 text-[11px] leading-relaxed">
                    Your keys are encrypted with AES-256-GCM and stored securely on our server.
                    They are only decrypted in memory when executing trades — never logged or exposed.
                    Claudia can&apos;t withdraw — only trade. Revoke your key anytime from your exchange settings.
                  </p>
                </div>
              </div>
            </div>
